...
class NN:
    ...
    def backpropagation(self,X,target,m,n,limit):
        one_hot_target = self.one_hot(target,limit).T
    ...
    def predict(self):
        return np.argmax(self.output_activation.output,0)
...
neural_network = NN(784,20,20,15,15,12,12,10)
...
while True:
    ...
    loss = neural_network.backpropagation(X_train,Y_train,m,n,10)
    neural_network.update_weights_and_biases()
    
    print(f"Loss of iteration {iteration} is = {loss}")
    print()
